...
public class GridManager : MonoBehaviour {
    ...
    public void SetAsType(Vector3 pos, TileType type)
    {
    ...
    public void BuildWorld(int n_rows, int n_cols)
    {
        ...
        Debug.Log("row : " + n_rows + "col : " + n_cols);
    ...
    int[] findAstarPath(int from, int to, TileType[] world)
    {
        ...
        while (open.Count > 0) 
        {
            ...
            foreach (var neighbor in neighbors)
            {
                ...
                if (world[neighbor] == TileType.Plain)
                    g = current.g + computeDistance(current.no, neighbor);
                else if(world[neighbor] == TileType.Swamp)
                    g = current.g + computeDistance(current.no, neighbor) * 2.0f;
    ...
    // 몬스터들이 움직이는 것으로 변경한 버전
    public IEnumerator MonsterMove(GameObject player, Vector3 destination)
    {
        int start = pos2Cell(player.transform.position);
        int end = pos2Cell(destination);
        int[] path = null;
    ...
    public void ChangeTileType(Vector3 pos, TileType type)
    {
        world[pos2Cell(pos)] = type;
    }
}
